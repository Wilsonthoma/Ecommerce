// src/pages/Deals.jsx - COMPLETE with Yellow-Orange Theme
...
const Deals = () => {
  ...
  useEffect(() => {
    const fetchDeals = async () => {
      try {
        ...
        const flashResponse = await clientProductService.getFlashSaleProducts(8);
        if (flashResponse?.success) {
          setFlashDeals(flashResponse.products || []);
        }

        // Fetch daily deals (products with discount)
        const dailyResponse = await clientProductService.getProducts({ 
          limit: 8, 
        ...
        const clearanceResponse = await clientProductService.getProducts({ 
          limit: 8, 
          sort: '-discountPercentage'
        });
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <PageHeader 
        title="HOT DEALS" 
        subtitle="Amazing discounts waiting for you"
        image={dealsHeaderImage}
      />
